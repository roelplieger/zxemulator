...
public class Z80ServiceImpl implements Z80Service {
	...
	private byte and(byte x, byte y) {
		byte result = (byte)((x & 0xff) & (y & 0xff));
	...
	private synchronized void doStep() throws MemoryException, PortException {
		...
		switch (op) {
			...
			case 0xDB:
				...
				clockCycles = 11;
				A = registerService.getA();
				port = ((A << 8) & 0xffff) + (memoryService.readByte(PC + 1) & 0xff);
	...
	private short doIXIY(short PC, int mode) throws MemoryException, PortException {
		...
		switch (op) {
			...
			case 0x24:
				...
				clockCycles = 8;
				value = getIXIY(mode);
				byte byteValue = inc((byte)((value & 0xff00) >> 8));
				value = (short)((value & 0xff) + ((byteValue & 0xff) << 8));
				setIXIY(mode, value);
				PC += 2;
				break;
			...
			case 0x25:
				// dec ixh / dec iyh
				value = getIXIY(mode);
				byteValue = dec((byte)((value & 0xff) >> 8));
				value = (short)((value & 0xff) + ((byteValue & 0xff) << 8));
				setIXIY(mode, value);
				PC += 2;
				break;
			...
			case 0x26:
				...
				value = (short)((value & 0xff00) + (byteValue & 0xff));
				setIXIY(mode, value);
				PC += 3;
				break;
			...
			case 0x45:
				// ld b,ixl / ld b,iyl
				clockCycles = 8;
				byteValue = (byte)(getIXIY(mode) & 0xff);
				registerService.setB(byteValue);
				PC += 2;
				break;
			...
			case 0x54:
				// ld d,ixh / ld d,iyh
				clockCycles = 8;
				byteValue = (byte)((getIXIY(mode) >> 8) & 0xff);
				registerService.setD(byteValue);
				PC += 2;
				break;
			...
			case 0x84:
				// add a,ixh / add a,iyh
				clockCycles = 8;
				byteValue = (byte)((getIXIY(mode) & 0xff) >> 8);
				byte A = registerService.getA();
